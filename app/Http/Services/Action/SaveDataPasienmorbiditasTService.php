...
class SaveDataPasienmorbiditasTService
{
        ...
        if($konfig->is_updatediagnosacasemix){
            try {             
                ...
                $preparedData = [
                    ...
                    'pegawai_id' => $dataDiagnosa['pegawai_id'] ?? null,
                    'diagnosa_id' => $dataDiagnosa['diagnosa_id'] ?? null,
                    'diagnosa_nama' => $dataDiagnosa['diagnosa_nama'] ?? null,
                    'diagnosa_kode' => $dataDiagnosa['diagnosa_kode'] ?? null,
                    ...
                    'kelompokumur_id' => $RegisterData['kelompokumur_id'],
                    'golonganumur_id' =>$RegisterData['golonganumur_id'],
                    'pasienmorbiditas_id' =>null,

                    // Tambahkan field lain sesuai kebutuhan
                ];
            return response()->json(['success' => true,'message' => 'Data berhasil disimpan','pasienmorbiditasT'=>$preparedData], 200);
    ...
    public function addDataPasienMordibitasR(array $dataDiagnosa = []){
        ...
        if(!empty($dataDiagnosa['pasienmorbiditas_id'])){
