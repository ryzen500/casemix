...
export default class DataTable extends Component {
  ...
  pagesNumbers() {
    ...
    if (to >= this.state.entities.pagination.total_pages) {
      to = this.state.entities.pagination.total_pages;
    }
    let pagesArray = [];
  ...
  pageList() {
    return this.pagesNumbers().map(page => {
      return <li className={ page === this.state.entities.pagination.current_page ? 'page-item active' : 'page-item' } key={page}>
            ...
            <ul className="pagination">
              <li className="page-item">
                <button className="page-link"
                  disabled={ 1 === this.state.entities.pagination.current_page }
                  onClick={() => this.changePage(this.state.entities.pagination.current_page - 1)}
                >
                  Previous
                </button>
              </li>
              ...
              <li className="page-item">
                <button className="page-link"
                  disabled={this.state.entities.pagination.total_pages === this.state.entities.pagination.current_page}
                  onClick={() => this.changePage(this.state.entities.pagination.current_page + 1)}
                >
                  Next
                </button>
              </li>
              <span style={{ marginTop: '8px' }}> &nbsp; <i>Displaying { this.state.entities.data.length } of { this.state.entities.pagination.total_items } entries.</i></span>
            </ul>
          </nav>
        }
      </div>
